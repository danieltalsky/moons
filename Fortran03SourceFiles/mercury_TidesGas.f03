...
!------------------------------------------------------------------------------
   ...
   subroutine mfo_user(time, jcen, nbod, nbig, m, x, v, a)
      !
      use mercury_constants
      implicit none
      !
      ! Input/Output
      integer nbod, nbig
      real(kind=8) time, jcen(3), m(nbod), x(3, nbod), v(3, nbod), a(3, nbod)
      !  **************   DGV Dimitri Veras added these parameters ************
      real(kind=8) Omegag, rhog, delta
      ...
      ! Local
      integer j
      !  **************   ##R## Rachel Worth added these parameters ************
      real(kind=8) aa(3, nbod), ab(3, nbod), cm(3)
      real(kind=8) mtot, mgas0, mgas, rgas, gaslife, gasfrac
      !
      ...
      !
      !  **************   ##R## Rachel Worth added this subroutine ************
      !------------------------------------------------------------------------------
...
150   continue
  ...
  &   nhit, jhit, thit, dhit, algor, ngf, ngflag)
      ...
      real(kind=8) hx, hy, hz, h2, p, rr0, rr1, rv0, rv1, temp, e, v2
      real(kind=8) xu0(3, NMAX), xu1(3, NMAX), vu0(3, NMAX), vu1(3, NMAX)
      ! ##R## v0^2, needed for collision criterion modification
      real(kind=8) vv0
      ! ##R##
      !
      !------------------------------------------------------------------------------
      ...
      do j = i0, nbod
         ...
         if ((rv0*h <= 0 .and. rv1*h >= 0) .or. min(rr0, rr1) <= rcen2) then
            ...
            !          if (q<=rcen) then
            ! ##R## New collision criteria (must be within timestep*velocity of center)
...
230                  open (23, file=outfile, status='old', access='append', &
  ...
  &   ngflag, opt)
      ...
      real(kind=8) ha(2), hb(2), rt10, angf(3, NMAX), ausr(3, NMAX)
      !
      !------------------------------------------------------------------------------
      !
      rt10 = sqrt(10.d0)
      ...
      do k = 1, 2
       ...
       &       vh(3, j), ha(k), iflag)
         ...
         if (opt(8) == 1) call mfo_user(time, jcen, nbod, nbig, m, xh, vh, &
      &     ausr)
  ...
  &   ngflag, opt)
      ...
      do k = 1, 2
       ...
       &       v(3, j), ha(k), iflag)
         ...
         !
         ! If required, apply non-gravitational and user-defined forces
         if (opt(8) == 1) call mfo_user(time, jcen, nbod, nbig, m, x, v, ausr)
...
100   call drift_kepu(dt, r0, mu, alpha, u, fp, c1, c2, c3, iflg)
   ...
   real(kind=8) function orbel_fhybrid(e, n) result(orbel_hybrid_result)
      ...
      if (abn < 0.636d0*e - 0.6d0) then
         orbel_hybrid_result = orbel_flon(e, n)
      else
         orbel_hybrid_result = orbel_fget(e, n)
      end if
...
end module mercury_utilities

program mercury_tides_gas_version
...
!
!------------------------------------------------------------------------------
! Bug fix: Rachel Worth
!
! The criteria for determining whether a body has collided with the central
! object worked poorly for moons/satellites. I have added the requirement that
! in order for a collision to be scored, the body must have been able to reach
! the center at the velocity it was going, giving its location.
!
! The alteration is in the mce_cent subroutine and is marked with ##R##.
!
!------------------------------------------------------------------------------
!
! Some of Avi Mandell's changes are included here and are marked with
...
end program mercury_tides_gas_version
