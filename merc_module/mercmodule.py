...
class MercModule:
    ...
    def ReadInfo(whichdir):
        ...
        while not footer:  # body of file
            ...
            if len(splitline) == 8 and splitline[0] != "Continuing":
                name1[j - hlen], dest1[j - hlen], time1[j - hlen] = (
                    splitline[4],
                    splitline[0],
                    splitline[6],
                )
    ...
    def CopyInfo(
        ...
        """Read collision information from the specified directory, summarize it, and update output files.

        Parameters
        ----------
        whichdir : str
            Path to the directory containing the simulation input/output files.
        whichtime : str
            A string representation of the time or label associated with this simulation step.
        writegood : bool
            If True, writes records of certain collision events (e.g., impacting Jupiter, Moon) to ``good.in``.
        writeelem : bool
            If True, updates ``element.in`` with default planetary data and, if applicable, information about special collisions.

        """
        assert type(whichdir) is str
        assert type(whichtime) is str
        assert type(writegood) is bool
        assert type(writeelem) is bool
            ...
            if len(name1) > 0:
                ...
                if writegood:
                    ...
                    for j in range(5, SmallLen):
                        smalllines[j] = smallin.readline()
                        if any(
                            name[i] == smalllines[k].split()[0]
                            and float(time1[i]) > 60.0
    ...
    def MakeMoon(whichdir, whichtime):
        ...
        BigFirstLines = [
            big[i].ljust(10) + "d= " + dpl[i] + "  m= " + mpl[i] + "\n"
        ...
        MercModule.WriteObjInFile(
            here, whichdir, big, "big", BigHeader, BigFirstLines, bigxv, bigs
        )
    ...
    def MakeBigRand(
        ...
        if seed:
            timestep = 5 + seed
            assert seed <= AEILen - 5, (
                f"Seed value '{seed}' exceeds maximum allowed value {AEILen - 5}"
            )
        else:
    ...
    def MakeSmall(whichdir, whichtime, n, whichpl, da, dv):
        ...
        SmallFirstData = [SmallFirstData[i].split() for i in range(len(SmallFirstData))]
        ...
        ### Format data as the first line of each big.in object entry
        SmallFirstLines = [
            small[i].ljust(10) + "d= " + d + "  m= " + m + "  r= 0.001\n"
    ...
    def Good2Small(whichdir, whichtime, n):
        ...
        # @TODO: Needs to be made an absolute path?  Looks like it used to use the subdirectory?
        ...
        ### Format data as the first line of each big.in object entry
        SmallFirstLines = [
            name[i].ljust(10) + "d= " + d + "  m= " + m + "  r= 0.001\n"
        ...
        ### Write data
        MercModule.WriteObjInFile(
            here, whichdir, name, "small", SmallHeader, SmallFirstLines, smallxv, smalls
        )
    ...
    def MakeSmallEjecta(
        ...
        Mplanet = (
            float(
                [x.strip() for x in planet_first_line if ("m=" in x)][0].replace(
                    "m=", ""
                )
            )
            * mSun
        ...
        Dplanet = float(
            [x.strip() for x in planet_first_line if ("d=" in x)][0].replace("d=", "")
        )
        ...
        Rhill = 1.0 * (Mplanet / mSun) ** (1.0 / 3.0)  # in AU
        vesc = (
            sqrt(2 * G * Mplanet / (1.1 * Rhill * AU + Rplanet)) * day / AU
        )  # in AU/day

        ### Assign random values for theta and phi
        theta = numpy.random.rand(n) * 360.0
        phi = numpy.random.rand(n) * 180.0
    ...
    def PlotAllObjects(
        ...
        for i, obj_file in enumerate(planet_objs + ejecta_objs):
            ...
            coords[obj] = pd.read_csv(
                sim_out_dir + obj + ".aei", sep=r"\s+", names=header, skiprows=4
            )
