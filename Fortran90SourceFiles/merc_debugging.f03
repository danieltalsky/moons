...
!
      ...
      real(kind=8) m(NMAX),xh(3,NMAX),vh(3,NMAX),s(3,NMAX),rho(NMAX)
      real(kind=8) rceh(NMAX),epoch(NMAX),ngf(4,NMAX),rmax,rcen,jcen(3)
      real(kind=8) cefac,time,tstart,tstop,dtout,h0,tol,en(3),am(3)
      character*8 id(NMAX)
      character*80 outfile(4), dumpfile(4), mem(NMESS)
...
!
! mal_hvar (time,tstart,tstop,dtout,algor,h0,tol,jcen,
...
! mal_hcon (time,tstart,tstop,dtout,algor,h0,tol,jcen,
!     %  rcen,rmax,en,am,cefac,ndump,nfun,nbod,nbig,m,xh,vh,s,rho,rceh,
!     %  stat,id,ngf,opt,opflag,ngflag,outfile,dumpfile,mem,lmem,onestep,
!     %  coord,bcoord)
! Main integration
      if (algor==1) call mal_hcon (time,tstart,tstop,dtout,algor,h0, &
...
!
      if (algor==9) call mal_hcon (time,tstart,tstop,dtout,algor,h0, &
     &   tol,jcen,rcen,rmax,en,am,cefac,ndump,nfun,nbod,nbig,m,xh,vh,s, &
     &   rho,rceh,stat,id,ngf,opt,opflag,ngflag,outfile,dumpfile,mem, &
     &   lmem,mdt_mvs,mco_iden,mco_iden)
!
      if (algor==2) call mal_hvar (time,tstart,tstop,dtout,algor,h0, &
     &   tol,jcen,rcen,rmax,en,am,cefac,ndump,nfun,nbod,nbig,m,xh,vh,s, &
     &   rho,rceh,stat,id,ngf,opt,opflag,ngflag,outfile,dumpfile,mem, &
     &   lmem,mdt_bs1)
!
      if (algor==3) call mal_hvar (time,tstart,tstop,dtout,algor,h0, &
     &   tol,jcen,rcen,rmax,en,am,cefac,ndump,nfun,nbod,nbig,m,xh,vh,s, &
     &   rho,rceh,stat,id,ngf,opt,opflag,ngflag,outfile,dumpfile,mem, &
     &   lmem,mdt_bs2)
!
      if (algor==4) call mal_hvar (time,tstart,tstop,dtout,algor,h0, &
     &   tol,jcen,rcen,rmax,en,am,cefac,ndump,nfun,nbod,nbig,m,xh,vh,s, &
     &   rho,rceh,stat,id,ngf,opt,opflag,ngflag,outfile,dumpfile,mem, &
     &   lmem,mdt_ra15)
!
      if (algor==10) call mal_hcon (time,tstart,tstop,dtout,algor,h0, &
     &   tol,jcen,rcen,rmax,en,am,cefac,ndump,nfun,nbod,nbig,m,xh,vh,s, &
     &   rho,rceh,stat,id,ngf,opt,opflag,ngflag,outfile,dumpfile,mem, &
...
! AVIMANDELL
!  *Closes output files stranded by comments
      close (21)
      close (22)
      close (23)
      close (24)
...
! AVIMANDELL
!  *Prints runtime in days
      call cpu_time(runtime)
      write (*,'(3x,a16,f9.6)') 'Runtime in days: ',runtime/86400.0
!
!------------------------------------------------------------------------------
!
...
!
! Applies drag forces to giant planet to simulate migration, and gas drag
! to small objects.
!
...
!------------------------------------------------------------------------------
!
      subroutine mfo_user (time,step,nbod,nbig,m,rphys,x,v,ausr)
!
      implicit none
      include 'mercury.inc'
      include 'user_gl581.inc'
!
! Input/Output
      integer nbod, nbig
      real(kind=8) time,step,m(nbod),x(3,nbod),v(3,nbod)
      real(kind=8) ausr(3,nbod),rphys(NMAX)
!
! Local
      integer j,k
!
      ...
      real(kind=8) rho0,z0,rhogas,mdrag,rdrag,kdrag,ndrag,rhodrag,rhoperc

      real(kind=8) vk,nu,vgas,th,vrel(3,nbod)

      real(kind=8) amig(3,nbod),adrag(3,nbod)
!------------------------------------------------------------------------------
!
!  SETUP
!
      ...
      enddo
!
!  GIANT PLANET MIGRATION DRAG
!
      if (nmig==0) goto 200
      if (aint(time)>=aint(tmax)) goto 200

      do j = 2,2+nmig
         if (j>1+nmig) goto 199

         R = sqrt(x(1,j)**2 + x(2,j)**2 + x(3,j)**2)
         if (R<=stopdist) goto 199

         if (aint(time)<aint(switch*tmax)) then
            tmig = migfact(j-1)*tmax*tmig0
         else
            tmig = migfact(j-1)*tmax*tmig0* &
     &               ((1.0-switch)/(1.0-((time+0.0001)/tmax)))**gamma
         endif

         amig(1,j) = -0.5*v(1,j)/(tmig-time+0.0001)
         amig(2,j) = -0.5*v(2,j)/(tmig-time+0.0001)
         amig(3,j) = -0.5*v(3,j)/(tmig-time+0.0001)
...
!------------------------------------------------------------------------------
!
!  SMALL OBJECT GAS DRAG
!
      if (rhoconst==0.0d0) goto 300
      if ((gastime/=0.0d0).and.(aint(time)>=aint(gastime))) goto 300

      if (gastime>0.0d0) then
         rhoperc = (gastime - time)/gastime
      else
         rhoperc = 1.0d0
      endif

      do j = 2,nbod
         if (m(j)==0.0d0) goto 250

         R = sqrt(x(1,j)**2 + x(2,j)**2) !AU
         rho0 = rhoconst*rhoperc*((R)**(-rhoexp))                     !g/cm^3
         z0 = zconst*((R)**(zexp))                                    !AU
         rhogas = rho0*exp((-1.0d0*(x(3,j)**2))/(z0**2))            !g/cm^3
         ...
         ndrag = (m(j)*MSUN)/(K2*mdrag)
         rdrag = (rphys(j)*AU)/(ndrag**(1.0/3.0))
         rhodrag = (3.0*mdrag)/(4.0*PI*(rdrag**3))
         kdrag = (AU*3.0d0*rhogas*cdrag)/(8.0d0*rhodrag*rdrag)        !1/AU

         vk = ((GCONST*(m(1)/K2)*R*R)**(1./2.)) / &
     &          (((R**2)+(x(3,j)**2))**(3./4.))                        !AU/day
         nu = (PI/16.0d0)*(rhoexp-1.0d0)*((z0/R)**2)
         vgas = vk*(1.0d0 - nu)                                       !AU/day

         th = atan(x(2,j)/x(1,j))
         if (x(1,j)<0.0d0) then
            th = PI + th
         else
            if (x(2,j)<0.0d0) th = (2.0d0*PI) + th
         endif

         vrel(1,j) = v(1,j) - (-1.0d0*vgas*sin(th))
         vrel(2,j) = v(2,j) - (vgas*cos(th))
         vrel(3,j) = v(3,j)

         do k = 1,3
            adrag(k,j)=-1.0d0*kdrag*sqrt(vrel(k,j)**2)*vrel(k,j) !AU/day^2

            if (adrag(k,j)/(-1.0d0*vrel(k,j)/step)>1.0d0) &
     &          adrag(k,j) = -1.0d0*vrel(k,j)/step
         enddo
...
!
 300  continue
      ...
      do j = 2,nbod
         ausr(1,j) = amig(1,j) + adrag(1,j)
         ausr(2,j) = amig(2,j) + adrag(2,j)
         ausr(3,j) = amig(3,j) + adrag(3,j)
      enddo
      return
      end
!
...
! Input/Output
      ...
      integer lmem(NMESS)
      real(kind=8) time,tstart,tstop,dtout,h0,tol,jcen(3),rcen,rmax
      real(kind=8) en(3),am(3),cefac,m(nbod),xh(3,nbod),vh(3,nbod)
      real(kind=8) s(3,nbod),rho(nbod),rceh(nbod),ngf(4,nbod)
      character*8 id(nbod)
      character*80 outfile(4),dumpfile(4),mem(NMESS)
!
! Local
      ...
      real(kind=8) tmp0,h,hdid,tout,tdump,tfun,tlog,tsmall,dtdump,dtfun
      real(kind=8) thit(CMAX),dhit(CMAX),thit1,x0(3,NMAX),v0(3,NMAX)
      real(kind=8) rce(NMAX),rphys(NMAX),rcrit(NMAX),a(NMAX)
      real(kind=8) dclo(CMAX),tclo(CMAX),epoch(NMAX)
      real(kind=8) ixvclo(6,CMAX),jxvclo(6,CMAX)
...
! Set up time of next output, times of previous dump, log and periodic effect
      if (opflag==-1) then
        ...
        if ((tstop - tstart)*(tout - tstop)>0) tout = tstop
      endif
      tdump = time
      tfun  = time
      tlog  = time
...
! Is it time for output ?
      if (abs(tout-time)<abs(tsmall).and.opflag>=-1) then
!
...
! If integration has finished return to the main part of programme
      if (abs(tstop-time)<=abs(tsmall).and.opflag/=-1) return
!
! Set the timestep
      if (opflag==-1) tmp0 = tstart - time
      if (opflag==-2) tmp0 = tstop  - time
      if (opflag>=0)  tmp0 = tout   - time
...
!
! Do the data dump
      if (abs(time-tdump)>=abs(dtdump).and.opflag>=-1) then
        do j = 2, nbod
          epoch(j) = time
        enddo
        call mio_ce (time,tstart,rcen,rmax,nbod,nbig,m,stat,id, &
...
! Write a progress report to the log file
      if (abs(time-tlog)>=abs(dtdump).and.opflag>=0) then
        call mxx_en (jcen,nbod,nbig,m,xh,vh,s,en(2),am(2))
        call mio_log (time,tstart,en,am,opt,mem,lmem,outfile)
        tlog = time
      endif
!
!------------------------------------------------------------------------------
...
!
      if (abs(time-tfun)>=abs(dtfun).and.opflag>=-1) then
!
...
! Local
      ...
      real(kind=8) x(3,NMAX),v(3,NMAX),xh0(3,NMAX),vh0(3,NMAX)
      real(kind=8) rce(NMAX),rphys(NMAX),rcrit(NMAX),epoch(NMAX)
      real(kind=8) hby2,tout,tmp0,tdump,tfun,tlog,dtdump,dtfun
      real(kind=8) dclo(CMAX),tclo(CMAX),dhit(CMAX),thit(CMAX)
      real(kind=8) ixvclo(6,CMAX),jxvclo(6,CMAX),a(NMAX)

! AVIMANDELL
!      real(kind=8) R0,delR,R,tmig, dummy1
      logical test
! ##R##
	integer loop
...
! ##R## positions should be the same still (this is before the loop)
!	print *, xh(1:3,1:7)
!	print *, vh(1:3,1:7)
	loop=0
...
! AVIMANDELL
!  *Init for the forced migration
!      R0 = sqrt(xh(1,nmig)**2 + xh(2,nmig)**2 + xh(3,nmig)**2)
!
...
! ##R##  check transformed coordinates; xh array only has 6 rows now but x is 3x50000; x=xh, v slightly different than vh
!	print *,shape(xh),shape(x)
!	print *,x(1:3,1:7)
!	print *,v(1:3,1:7)
...
!
! Is it time for output ?
      if (abs(tout-time)<=hby2.and.opflag>=-1) then
!
! Beware: the integration may change direction at this point!!!!
        if (opflag==-1.and.dtflag/=0) dtflag = 1
!
...
! If integration has finished, convert to heliocentric coords and return
      if (abs(tstop-time)<=hby2.and.opflag>=0) then
        call bcoord (time,jcen,nbod,nbig,h0,m,x,v,xh,vh,ngf,ngflag,opt)
	print *, 'end'
        return
      endif
...
! Main integration step
     ...
     &   mem,lmem)
      time = time + h0
! ##R## Check outputs:
!	print *,time,'d, Row 6 (x) = ',x(1,6)
! AVIMANDELL
!  *Forced migration routine
!      R = sqrt(x(1,nmig)**2 + x(2,nmig)**2 + x(3,nmig)**2)
!      if (aint(time)>=aint(tmax)) goto 200
!      if ((stopdist==0.0d0).and.(stoptime==0.0d0)) goto 199
!      if ((stopdist==0.0d0).and.(aint(time)<aint(stoptime)))
!     &  goto 199
!      if ((stopdist==0.0d0).and.(aint(time)>=aint(stoptime)))
!     &  goto 200
!      if ((stoptime==0.0d0).and.(R>stopdist)) goto 199
!      if ((stoptime==0.0d0).and.(R<=stopdist)) goto 200
!
! 199  continue
!      if (aint(time)<aint(switchtime)) then
!         tmig = tmig0
!      else
!         tmig = tmig0*((tmax-switchtime)/(tmax-time+0.0001))**gamma
!      endif
!      delR = stepdecr*(R0*(h0/tmig))
!
!      x(1,nmig) = x(1,nmig) - (x(1,nmig)*(delR/R))
!      x(2,nmig) = x(2,nmig) - (x(2,nmig)*(delR/R))
!      x(3,nmig) = x(3,nmig) - (x(3,nmig)*(delR/R))
!
! 200  continue
!
!
...
! Convert to heliocentric coords and do the data dump
      ...
      if (test.and. &
     &  (abs(time-tdump)>=abs(dtdump).and.opflag>=-1)) then
        call bcoord (time,jcen,nbod,nbig,h0,m,x,v,xh,vh,ngf,ngflag,opt)
        do j = 2, nbod
          epoch(j) = time
        enddo
     ...
     &     id,ngf,epoch,opt,opflag,dumpfile,mem,lmem)
        do j = 1, 4
           close (20+j)
 10        open (20+j,file=outfile(j),status='old', &
...
! Input/Output
      integer nbod
      real(kind=8) h,x0(3,nbod), x1(3,nbod), v0(3,nbod),v1(3,nbod)
      real(kind=8)   xmin(nbod), xmax(nbod), ymin(nbod),ymax(nbod)
!
! Local
      integer j
      real(kind=8) temp
...
! If velocity changes sign, do an interpolation
        if ((v0(1,j)<0.and.v1(1,j)>0).or. &
     &       (v0(1,j)>0.and.v1(1,j)<0)) then
...
!
        if ((v0(2,j)<0.and.v1(2,j)>0).or. &
     &       (v0(2,j)>0.and.v1(2,j)<0)) then
...
!
! Input/Output
      integer i0, nbod, nbig, nhit, jhit(CMAX), algor, ngflag,loop
      real(kind=8) time,h,rcen,jcen(3),m(nbod),x0(3,nbod),v0(3,nbod)
      real(kind=8) x1(3,nbod),v1(3,nbod),thit(CMAX),dhit(CMAX),ngf(4,nbod)
!
! Local
      integer j
      real(kind=8) rcen2,mco_acsh,a,q,u0,uhit,m0,mhit,mm,r0,mcen
      real(kind=8) hx,hy,hz,h2,p,rr0,rr1,rv0,rv1,vv0,temp,e,v2
      real(kind=8) xu0(3,NMAX),xu1(3,NMAX),vu0(3,NMAX),vu1(3,NMAX)
!
!------------------------------------------------------------------------------
!
! ##R##
!	print *,'mce_cent:',time
! x0: 3 pos coords for central body (all zero) and each other remaining body
!	print *, x0
      if (i0<=0) i0 = 2
      nhit = 0
      rcen2 = rcen * rcen
...
! If inside the central body, or passing through pericentre, use 2-body approx.
        if ((rv0*h<=0.and.rv1*h>=0).or.min(rr0,rr1)<=rcen2) then
          ...
          q = p / (1.d0 + e)
! ##R##
!	print *, 'h*sqrt(vv0)=',h*sqrt(vv0),', sqrt(rr0)=',sqrt(rr0)
!	print *,'       loop           j      '
!	print *, loop,j, x0(1,j) , v0(2,j) , x0(2,j) , v0(1,j)
!
! If the object hit the central body
!          if (q<=rcen) then
! ##R##
          if ((q<=rcen).and.((h*sqrt(vv0))>=(sqrt(rr0)))) then
            nhit = nhit + 1
            jhit(nhit) = j
            dhit(nhit) = rcen
!
...
! Input/Output
      ...
      real(kind=8) time,tstart,elost,jcen(3)
      real(kind=8) m(nbod),xh(3,nbod),vh(3,nbod),s(3,nbod),rphys(nbod)
...
! Input/Output
      integer nbod
      real(kind=8) m(nbod),x(3,nbod),v(3,nbod),hill(nbod),a(nbod)
!
! Local
      integer j
      real(kind=8) r, v2, gm
...
!
      ...
      include 'mercury.inc'
!
      real(kind=8) N2,THIRD
      parameter (N2=.4d0,THIRD=.3333333333333333d0)
!
! Input/Output
      integer nbod,nbig,algor,opt(8),rcritflag
      real(kind=8) tstart,h,jcen(3),rcen,rmax,cefac,m(nbod),x(3,nbod)
      real(kind=8) v(3,nbod),s(3,nbod),rho(nbod),rceh(nbod),rphys(nbod)
      real(kind=8) rce(nbod),rcrit(nbod)
      character*8 id(nbod)
      character*80 outfile
!
! Local
      integer j
      real(kind=8) a(NMAX),hill(NMAX),temp,amin,vmax,k_2,rhocgs,rcen_2
...
! Input/Output
      ...
      real(kind=8) jcen(3),m(nbod),xh(3,nbod),vh(3,nbod),s(3,nbod),elost
!
! Local
      integer k
      real(kind=8) tmp1, tmp2, dx, dy, dz, du, dv, dw, msum, mredu, msum_1
      real(kind=8) e0, e1, l2
!
!------------------------------------------------------------------------------
!
...
!
! Input/Output
      real(kind=8) d0,d1,d0t,d1t,h,d2min,tmin
!
! Local
      real(kind=8) a,b,c,temp,tau
!
!------------------------------------------------------------------------------
!
      if (d0t*h>0.or.d1t*h<0) then
        if (d0<=d1) then
          d2min = d0
          tmin = -h
        else
...
! Input/Output
      ...
      real(kind=8) x0(3,nbod),v0(3,nbod),x1(3,nbod),v1(3,nbod),h,rcrit(nbod)
!
! Local
      integer i,j
      real(kind=8) d0,d1,d0t,d1t,d2min,temp,tmin,rc,rc2
      real(kind=8) dx0,dy0,dz0,du0,dv0,dw0,dx1,dy1,dz1,du1,dv1,dw1
      real(kind=8) xmin(NMAX),xmax(NMAX),ymin(NMAX),ymax(NMAX)
!
!------------------------------------------------------------------------------
!
      if (i0<=0) i0 = 2
...
! Identify pairs whose X-Y boxes overlap, and calculate minimum separation
      do i = i0, nbig
        do j = i + 1, nbod
          if (xmax(i)>=xmin(j).and.xmax(j)>=xmin(i) &
     &       .and.ymax(i)>=ymin(j).and.ymax(j)>=ymin(i)) then
!
...
! If separation derivative changes sign, find the minimum separation
            d2min = HUGE
            if (d0t*h<=0.and.d1t*h>=0) call mce_min (d0,d1,d0t,d1t, &
     &         h,d2min,tmin)
!
...
! Input/Output
      ...
      real(kind=8) time,h,rcen,m(nbod),x0(3,nbod),v0(3,nbod)
      real(kind=8) x1(3,nbod),v1(3,nbod),rce(nbod),rphys(nbod)
      real(kind=8) dclo(CMAX),tclo(CMAX),thit(CMAX),dhit(CMAX),thit1
      real(kind=8) ixvclo(6,CMAX),jxvclo(6,CMAX)
      character*80 outfile,mem(NMESS)
!
! Local
      integer i,j
      real(kind=8) d0,d1,d0t,d1t,hm1,tmp0,tmp1
      real(kind=8) dx0,dy0,dz0,du0,dv0,dw0,dx1,dy1,dz1,du1,dv1,dw1
      real(kind=8) xmin(NMAX),xmax(NMAX),ymin(NMAX),ymax(NMAX)
      real(kind=8) d2min,d2ce,d2near,d2hit,temp,tmin
!
!------------------------------------------------------------------------------
!
      nhit = 0
...
! Check for close encounters between each pair of objects
      do i = 2, nbig
        do j = i + 1, nbod
          if (   xmax(i)>=xmin(j).and.xmax(j)>=xmin(i) &
     &       .and.ymax(i)>=ymin(j).and.ymax(j)>=ymin(i) &
     &       .and.stat(i)>=0.and.stat(j)>=0) then
!
...
! is in progress, store details
            if ((d2min<=d2ce.and.d0t*h<=0.and.d1t*h>=0) &
     &         .or.(d2min<=d2hit)) then
              nclo = nclo + 1
              if (nclo>CMAX) then
...
! Check for a near miss or collision
            if (d2min<=d2near) then
              ...
              chit(nhit) = -1
              if (d2min<=d2hit) chit(nhit) = 1
!
! Make sure the more massive body is listed first
              if (m(jhit(nhit))>m(ihit(nhit)).and.j<=nbig) then
                ihit(nhit) = j
                jhit(nhit) = i
              endif
!
! Is this the collision closest to the start of the time step?
              if ((tmin-thit1)*h<0) then
                thit1 = tmin
                nowflag = 0
                if (d1<=d2hit) nowflag = 1
              endif
            endif
          endif
!
...
! Input/Output
      ...
      real(kind=8) time,jcen(3),h,m(nbod),x(3,nbod),v(3,nbod),xh(3,nbod)
      real(kind=8) vh(3,nbod),ngf(4,nbod)
!
! Local
      integer j
!
...
!------------------------------------------------------------------------------
!
      subroutine mco_mvs2h (time,jcen,nbod,nbig,h,m,rphys,x,v,xh,vh,ngf, &
     &   ngflag,opt)
!
      implicit none
      include 'mercury.inc'
...
! Local
      ...
      real(kind=8) minside,msofar,gm(NMAX),a(3,NMAX),xj(3,NMAX),vj(3,NMAX)
      real(kind=8) ha(2),hb(2),rt10,angf(3,NMAX),ausr(3,NMAX),rphys(NMAX)
!
!------------------------------------------------------------------------------
!
      rt10 = sqrt(10.d0)
...
! If required, apply non-gravitational and user-defined forces
        if (opt(8)==1) call mfo_user (time,hb(k),nbod,nbig,m,rphys,xh, &
     &     vh,ausr)
        if (ngflag==1.or.ngflag==3) call mfo_ngf (nbod,xh,vh,angf, &
     &     ngf)
!
        do j = 2, nbod
...
!
! Input/Output
      real(kind=8) gm,q,e,i,p,n,l,x,y,z,u,v,w
!
! Local
      real(kind=8) g,a,ci,si,cn,sn,cg,sg,ce,se,romes,temp
      real(kind=8) z1,z2,z3,z4,d11,d12,d13,d21,d22,d23
      real(kind=8) mco_kep, orbel_fhybrid, orbel_zget
!
!------------------------------------------------------------------------------
!
...
! Input/Output
      integer nbod,nbig,ngflag,opt(8)
      real(kind=8) time,jcen(3),h,m(nbod),xh(3,nbod),vh(3,nbod),x(3,nbod)
      real(kind=8) v(3,nbod),ngf(4,nbod)
!
! Local
      integer j
      real(kind=8) mtot,temp
!
!------------------------------------------------------------------------------
!
      mtot = 0.d0
...
!------------------------------------------------------------------------------
!
      subroutine mco_h2mvs (time,jcen,nbod,nbig,h,m,rphys,xh,vh,x,v,ngf, &
     &   ngflag,opt)
!
      implicit none
      include 'mercury.inc'
...
!
! If required, apply non-gravitational and user-defined forces
        if (opt(8)==1) call mfo_user (time,hb(k),nbod,nbig,m,rphys,x, &
     &     v,ausr)
        if (ngflag==1.or.ngflag==3) call mfo_ngf (nbod,x,v,angf,ngf)
!
        do j = 2, nbod
...
! Input/Output
      integer nbod,nbig,ngflag,opt(8)
      real(kind=8) time,jcen(3),h,m(nbig),xh(3,nbig),vh(3,nbig),x(3,nbig)
      real(kind=8) v(3,nbig),ngf(4,nbod)
!
! Local
      integer j
      real(kind=8) mtot, mx, my, mz, mu, mv, mw, temp
!
!------------------------------------------------------------------------------c
      mtot = m(2)
      x(1,2) = xh(1,2)
...
!
! Input/Outout
      real(kind=8) oldl,e,mco_kep
!
! Local
      real(kind=8) l,pi,twopi,piby2,u1,u2,ome,sign
      real(kind=8) x,x2,sn,dsn,z1,z2,z3,f0,f1,f2,f3
      real(kind=8) p,q,p2,ss,cc
...
!
! Input/Output
      real(kind=8) gm,x,y,z,u,v,w,a,r,v2
!
!------------------------------------------------------------------------------
!
      r  = sqrt(x * x  +  y * y  +  z * z)
...
!
! Input/Output
      real(kind=8) rcen,rmax,mcen,m,x,y,z,u,v,w,fr,theta,phi,fv,vtheta,vphi
!
! Local
      real(kind=8) r,v2,v1,be,ke,temp
!
!------------------------------------------------------------------------------
!
        r = sqrt(x*x + y*y + z*z)
...
!
! Local
      real(kind=8) hx,hy,hz,h2,h,v2,r,rv,s,true
      real(kind=8) ci,to,temp,tmp2,bige,f,cf,ce
!
!------------------------------------------------------------------------------
!
      hx = y * w  -  z * v
...
!
      ...
      include 'mercury.inc'
!
      real(kind=8) SHRINK,GROW
      parameter (SHRINK=.55d0,GROW=1.3d0)
!
! Input/Output
      integer nbod, nbig, opt(8), stat(nbod), dtflag, ngflag
      integer nce, ice(nce), jce(nce)
      real(kind=8) time,h0,hdid,tol,jcen(3),mass(nbod),x0(3,nbod),v0(3,nbod)
      real(kind=8) s(3,nbod),ngf(4,nbod),rphys(nbod),rcrit(nbod)
      external force
!
! Local
      integer j, j1, k, n
      real(kind=8) tmp0,tmp1,tmp2,errmax,tol2,h,hx2,h2(8)
      real(kind=8) x(3,NMAX),v(3,NMAX),xend(3,NMAX),vend(3,NMAX)
      real(kind=8) a(3,NMAX),a0(3,NMAX),d(6,NMAX,8),xscal(NMAX),vscal(NMAX)
!
!------------------------------------------------------------------------------
!
      tol2 = tol * tol
...
! Local
      integer j,j1,k,n
      real(kind=8) tmp0,tmp1,tmp2,errmax,tol2,h,h2(12),hby2,h2by2
      real(kind=8) xend(3,NMAX),b(3,NMAX),c(3,NMAX)
      real(kind=8) a(3,NMAX),a0(3,NMAX),d(6,NMAX,12),xscal(NMAX),vscal(NMAX)
!
!------------------------------------------------------------------------------
!
      tol2 = tol * tol
...
! Input/Output
      ...
      real(kind=8) time,tstart,h0,tol,rmax,en(3),am(3),jcen(3),rcen
      real(kind=8) m(nbod),x(3,nbod),v(3,nbod),s(3,nbod),rphys(nbod)
      real(kind=8) rce(nbod),rcrit(nbod),ngf(4,nbod),tclo(CMAX),dclo(CMAX)
      real(kind=8) ixvclo(6,CMAX),jxvclo(6,CMAX)
...
! Local
      ...
      real(kind=8) a(3,NMAX),hby2,hrec,x0(3,NMAX),v0(3,NMAX),mvsum(3),temp
      real(kind=8) angf(3,NMAX),ausr(3,NMAX)
...
! If required, apply non-gravitational and user-defined forces
        if (opt(8)==1) call mfo_user (time,hby2,nbod,nbig,m,rphys,x,v, &
     &     ausr)
        if (ngflag==1.or.ngflag==3) call mfo_ngf (nbod,x,v,angf,ngf)
      endif
!
...
! Input/Output
      ...
      integer jclo(CMAX)
      real(kind=8) time,tstart,h0,hrec,tol,rmax,elost,jcen(3),rcen
      real(kind=8) m(nbod),x(3,nbod),v(3,nbod),s(3,nbod)
      real(kind=8) rce(nbod),rphy(nbod),rcrit(nbod),ngf(4,nbod)
      real(kind=8) tclo(CMAX),dclo(CMAX),ixvclo(6,CMAX),jxvclo(6,CMAX)
      character*80 outfile(4),mem(NMESS)
      character*8 id(nbod)
      external force
!
! Local
      ...
      real(kind=8) tlocal,hlocal,hdid,tmp0
      real(kind=8) mbs(NMAX),xbs(3,NMAX),vbs(3,NMAX),sbs(3,NMAX)
      real(kind=8) rcritbs(NMAX),rcebs(NMAX),rphybs(NMAX)
      real(kind=8) ngfbs(4,NMAX),x0(3,NMAX),v0(3,NMAX)
      real(kind=8) thit(CMAX),dhit(CMAX),thit1,temp
...
! Begin the Bulirsch-Stoer integration
  50  continue
        ...
        hrec = hlocal
        if (abs(hlocal)>tmp0) hlocal = sign (tmp0, h0)
!
! Save old coordinates and integrate
        call mco_iden (time,jcen,nbs,0,h0,mbs,xbs,vbs,x0,v0,ngf,ngflag, &
     &     opt)
        call mdt_bs1 (time,hlocal,hdid,tol,jcen,nbs,nbsbig,mbs,xbs,vbs, &
...
! Local
      ...
      real(kind=8) xj(3,NMAX),vj(3,NMAX),a(3,NMAX),gm(NMAX),hby2,thit1,temp
      real(kind=8) msofar,minside,x0(3,NMAX),v0(3,NMAX),dhit(CMAX),thit(CMAX)
      real(kind=8) angf(3,NMAX),ausr(3,NMAX)
!
!------------------------------------------------------------------------------
!
...
! Input/Output
      ...
      integer nce,ice(nce),jce(nce)
      real(kind=8) time,t,tdid,tol,jcen(3),mass(nbod)
      real(kind=8) x1(3*nbod),v1(3*nbod),spin(3*nbod)
      real(kind=8) ngf(4,nbod),rphys(nbod),rcrit(nbod)
      external force
!
! Local
      integer nv,niter,j,k,n
      real(kind=8) x(3*NMAX),v(3*NMAX),a(3*NMAX),a1(3*NMAX)
      real(kind=8) g(7,3*NMAX),b(7,3*NMAX),e(7,3*NMAX)
      real(kind=8) h(8),xc(8),vc(7),c(21),d(21),r(28),s(9)
      real(kind=8) q,q2,q3,q4,q5,q6,q7,temp,gk
!
!------------------------------------------------------------------------------
!
...
! Input/Output
      ...
      real(kind=8) time,jcen(3),m(nbod),x(3,nbod),v(3,nbod),s(3,nbod)
      real(kind=8) a(3,nbod),ngf(4,nbod),rcrit(nbod)
!
! Local
      integer j
      real(kind=8) acor(3,NMAX),acen(3)
!
!------------------------------------------------------------------------------
!
...
! Correct for oblateness of the central body
      if (jcen(1)/=0.or.jcen(2)/=0.or.jcen(3)/=0) then
...
! AVIMANDELL
!  Commented to avoid complicated work
!      if (opt(8)==1) then
!        call mfo_user (time,nbod,nbig,m,x,v,acor)
!        do j = 2, nbod
!          a(1,j) = a(1,j) + acor(1,j)
!          a(2,j) = a(2,j) + acor(2,j)
!          a(3,j) = a(3,j) + acor(3,j)
...
! Input/Output
      ...
      real(kind=8) m(nbod), x(3,nbod), v(3,nbod), a(3,nbod)
!
! Local
      integer i, j
      real(kind=8) sx, sy, sz, dx, dy, dz, tmp1, tmp2, s_1, s2, s_3, r3(NMAX)
!
!------------------------------------------------------------------------------
!
      sx = 0.d0
...
! Input/Output
      ...
      real(kind=8) m(nbod), x(3,nbod), a(3,nbod), rcrit(nbod)
!
! Local
      integer i,j
      real(kind=8) dx,dy,dz,s,s_1,s2,s_3,rc,rc2,q,q2,q3,q4,q5,tmp2,faci,facj
!
!------------------------------------------------------------------------------
!
      if (i0<=0) i0 = 2
...
! Input/Output
      integer nbod, nbig, stat(nbod)
      real(kind=8) jcen(3), m(nbod), x(3,nbod), a(3,nbod), rcrit(nbod)
!
! Local
      integer k
      real(kind=8) aobl(3,NMAX),acen(3)
!
!------------------------------------------------------------------------------
!
...
! Input/Output
      ...
      real(kind=8) time,jcen(3),rcrit(nbod),ngf(4,nbod),m(nbod)
      real(kind=8) x(3,nbod),v(3,nbod),a(3,nbod),spin(3,nbod)
!
! Local
      integer i, j, k
      real(kind=8) tmp2,dx,dy,dz,s,s_1,s2,s_3,faci,facj,rc,rc2,q,q2,q3,q4,q5
!
!------------------------------------------------------------------------------
!
...
! Input/Output
      integer nbod, nbig, stat(nbod)
      real(kind=8) jcen(3), m(nbod), x(3,nbod), xj(3,nbod), a(3,nbod)
!
! Local
      integer i,j,k,k1
      real(kind=8) fac0,fac1,fac12,fac2,minside,dx,dy,dz,s_1,s2,s_3,faci,facj
      real(kind=8) a0(3),a0tp(3),a1(3,NMAX),a2(3,NMAX),a3(3,NMAX),aobl(3,NMAX)
      real(kind=8) r,r2,r3,rj,rj2,rj3,q,q2,q3,q4,q5,q6,q7,acen(3)
!
!------------------------------------------------------------------------------
!
...
! Input/Output
      integer nbod
      real(kind=8) x(3,nbod), v(3,nbod), a(3,nbod), ngf(4,nbod)
!
! Local
      integer j
      real(kind=8) r2,r,rv,q,g,tx,ty,tz,nx,ny,nz,a1,a2,a3
!
!------------------------------------------------------------------------------
!
      do j = 2, nbod
...
! or if the non-gravitational force parameters are exceptionally large.
        if (r2<88.d0.or.abs(ngf(1,j))>1d-7 &
     &     .or.abs(ngf(2,j))>1d-7.or.abs(ngf(3,j))>1d-7) then
          r = sqrt(r2)
...
! Input/Output
      integer nbod
      real(kind=8) jcen(3), m(nbod), x(3,nbod), a(3,nbod), acen(3)
!
! Local
      integer i
      real(kind=8) jr2,jr4,jr6,r2,r_1,r_2,r_3,u2,u4,u6,tmp1,tmp2,tmp3,tmp4
!
!------------------------------------------------------------------------------
!
      acen(1) = 0.d0
...
! Input/Output
      integer nbod, nbig
      real(kind=8) m(nbod), x(3,nbod), v(3,nbod), a(3,nbod), ngf(4,nbod)
!
! Local
      integer j
!
...
!
! Input/Output
      real(kind=8) mio_c2fl
      character*8 c
!
! Local
      integer ex
      real(kind=8) x,mio_c2re
!
!------------------------------------------------------------------------------
!
...
! Input/output
      integer nchar
      real(kind=8) xmin,xmax,mio_c2re
      character*8 c
!
! Local
      integer j
...
! Input/Output
      ...
      real(kind=8) time,tstart,rcen,rmax,m(nbod),tclo(nclo),dclo(nclo)
      real(kind=8) ixvclo(6,nclo),jxvclo(6,nclo)
      character*80 outfile(4),mem(NMESS)
      character*8 id(nbod)
!
! Local
      integer k,year,month
      real(kind=8) tmp0,t1,rfac,fr,fv,theta,phi,vtheta,vphi
      character*80 c(200)
      character*38 fstop
...
!
      if (nstored>=100.or.ceflush==0) then
...
! Input/Output
      ...
      integer lmem(NMESS)
      real(kind=8) time,tstart,tstop,dtout,h0,tol,rmax,en(3),am(3)
      real(kind=8) jcen(3),rcen,cefac,m(nbod),x(3,nbod),v(3,nbod)
      real(kind=8) s(3,nbod),rho(nbod),rceh(nbod),ngf(4,nbod),epoch(nbod)
      character*80 dumpfile(4),mem(NMESS)
      character*8 id(nbod)
!
! Local
      integer idp,i,j,k,len,j1,j2
      real(kind=8) rhocgs,k_2,rcen_2,rcen_4,rcen_6,x0(3,NMAX),v0(3,NMAX)
      character*150 c
!
!------------------------------------------------------------------------------
!
...
!      if (algor==11) call mco_h2ub (time,jcen,nbod,nbig,h0,m,x,v,
!     %   x0,v0)
!
...
! Write header lines, data style (and epoch for Big bodies)
          ...
          if (i==1) write (31,*) mem(157)(1:lmem(157)),time
          write (31,'(a)') mem(155)(1:lmem(155))
!
! For each body...
          do j = j1, j2
...
! Create new version of the restart file
  60    if (idp==1) open (35, file='restart.tmp', status='unknown', &
     &     err=60)
...
! Input/Output
      ...
      integer lmem(NMESS),ndump,nfun
      real(kind=8) time,tstart,tstop,dtout,h0,tol,rmax,rcen,jcen(3)
      real(kind=8) en(3),am(3),m(NMAX),x(3,NMAX),v(3,NMAX),s(3,NMAX)
      real(kind=8) rho(NMAX),rceh(NMAX),epoch(NMAX),ngf(4,NMAX),cefac
      character*80 outfile(4),dumpfile(4), mem(NMESS)
      character*8 id(NMAX)
!
! Local
      integer j,k,itmp,jtmp,informat,lim(2,10),nsub,year,month,lineno
      real(kind=8) q,a,e,i,p,n,l,temp,tmp2,tmp3,rhocgs,t1,tmp4,tmp5,tmp6
!      real(kind=8) v0(3,NMAX),x0(3,NMAX)
...
! Input files
      do j = 1, 3
        ...
        do k = 1, j - 1
          if (infile(j)==infile(k)) call mio_err (6,mem(81),lmem(81), &
...
! Output files
      do j = 1, 4
        ...
        do k = 1, j - 1
          if (outfile(j)==outfile(k)) call mio_err (6,mem(81), &
     &       lmem(81),mem(89),lmem(89),outfile(j),80,mem(86),lmem(86))
        enddo
        do k = 1, 3
          if (outfile(j)==infile(k)) call mio_err (6,mem(81),lmem(81), &
...
! Dump files
      do j = 1, 4
        ...
        do k = 1, j - 1
          if (dumpfile(j)==dumpfile(k)) call mio_err (6,mem(81), &
     &       lmem(81),mem(89),lmem(89),dumpfile(j),80,mem(86),lmem(86))
        enddo
        do k = 1, 3
          if (dumpfile(j)==infile(k)) call mio_err (6,mem(81), &
     &       lmem(81),mem(89),lmem(89),dumpfile(j),80,mem(86),lmem(86))
        enddo
        do k = 1, 3
          if (dumpfile(j)==outfile(k)) call mio_err (6,mem(81), &
     &       lmem(81),mem(89),lmem(89),dumpfile(j),80,mem(86),lmem(86))
        enddo
      enddo
      close (15)
...
! Check if information file exists, and append a continuation message
     ...
     &      lmem(88),' ',1,outfile(j),80)
 320       open(20+j,file=outfile(j),status='old',access='append', &
     &         err=320)
        enddo
      else
!
! If new integration, check information file doesn't exist, and then create it
     ...
     &     ' ',1,outfile(3),80)
 410      open(20+j, file = outfile(j), status = 'new', err=410)
        enddo
!	close (24)
      endif
!
...
! Read integration parameters
  ...
  40    lineno = lineno + 1
        ...
        if (j==1) then
          ...
          enddo
          if (algor==0) call mio_err (23,mem(81),lmem(81),mem(98), &
     ...
     &       lim(1,nsub)+1,mem(85),lmem(85))
        ...
        if (j==5) read (c80,*,err=661) h0
        if (j==6) read (c80,*,err=661) tol
        c1 = c80(1:1)
        if (j==7.and.(c1=='y'.or.c1=='Y')) opt(1) = 1
        if (j==8.and.(c1=='n'.or.c1=='N')) opt(2) = 0
        if (j==9.and.(c1=='y'.or.c1=='Y')) opt(2) = 2
        if (j==10.and.(c1=='d'.or.c1=='D')) opt(3) = 0
        if (j==11.and.(c1=='y'.or.c1=='Y')) opt(3) = opt(3) + 2
        if (j==12) then
          if(c1=='l'.or.c1=='L') then
            opt(4) = 1
          else if (j==12.and.(c1=='m'.or.c1=='M')) then
            opt(4) = 2
          else if (j==12.and.(c1=='h'.or.c1=='H')) then
            opt(4) = 3
          else
            goto 661
          endif
        endif
	if (j==13.and.(c1=='y'.or.c1=='Y')) opt(5) = 1
        if (j==15.and.(c1=='y'.or.c1=='Y')) opt(8) = 1
        if (j==16) read (c80,*,err=661) rmax
        if (j==17) read (c80,*,err=661) rcen
        if (j==18) read (c80,*,err=661) m(1)
        if (j==19) read (c80,*,err=661) jcen(1)
        if (j==20) read (c80,*,err=661) jcen(2)
        if (j==21) read (c80,*,err=661) jcen(3)
...
! Read data style
 120    read (11,'(a150)') string
        if (string(1:1)==')') goto 120
        call mio_spl (150,string,nsub,lim)
        c3 = string(lim(1,nsub):(lim(1,nsub)+2))
        if (c3=='Car'.or.c3=='car'.or.c3=='CAR') then
          informat = 1
        else if (c3=='Ast'.or.c3=='ast'.or.c3=='AST') then
          informat = 2
        else if (c3=='Com'.or.c3=='com'.or.c3=='COM') then
          informat = 3
        else
...
! Read information for each object
 130    read (11,'(a)',end=140) string
        if (string(1:1)==')') goto 130
        call mio_spl (150,string,nsub,lim)
        if (lim(1,1)==-1) goto 140
!
! Determine the name of the object
        nbod = nbod + 1
        if (nbod>NMAX) call mio_err (23,mem(81),lmem(81),mem(90), &
...
! Check if another object has the same name
        do k = 1, nbod - 1
          if (id(k)==id(nbod)) call mio_err (23,mem(81),lmem(81), &
...
! Read values of mass, close-encounter limit, density etc.
        do k = 3, nsub, 2
          ...
          if (c80(1:1)=='m'.or.c80(1:1)=='M') then
            m(nbod) = temp * K2
          else if (c80(1:1)=='r'.or.c80(1:1)=='R') then
            rceh(nbod) = temp
          else if (c80(1:1)=='d'.or.c80(1:1)=='D') then
            rho(nbod) = temp * rhocgs
          else if (m(nbod)<0.or.rceh(nbod)<0.or.rho(nbod)<0) &
     &       then
            call mio_err (23,mem(81),lmem(81),mem(97),lmem(97),id(nbod), &
     &         8,mem(82+j),lmem(82+j))
          else if (c80(1:2)=='ep'.or.c80(1:2)=='EP'.or.c80(1:2) &
     &         =='Ep') then
            epoch (nbod) = temp
          else if (c80(1:2)=='a1'.or.c80(1:2)=='A1') then
            ngf (1,nbod) = temp
          else if (c80(1:2)=='a2'.or.c80(1:2)=='A2') then
            ngf (2,nbod) = temp
          else if (c80(1:2)=='a3'.or.c80(1:2)=='A3') then
            ngf (3,nbod) = temp
          else if (c80(1:1)=='b'.or.c80(1:1)=='B') then
            ngf (4,nbod) = temp
          else
            goto 666
          endif
...
! Alternatively, read Cometary or asteroidal elements
          ...
          if (algor==11.and.nbod/=2) temp = temp + m(2)
...
! Set non-gravitational-forces flag, NGFLAG
      ngflag = 0
      do j = 2, nbod
        if (ngf(1,j)/=0.or.ngf(2,j)/=0.or.ngf(3,j)/=0) then
          if (ngflag==0) ngflag = 1
          if (ngflag==2) ngflag = 3
        else if (ngf(4,j)/=0) then
          if (ngflag==0) ngflag = 2
...
! Write integration parameters to information file
        ...
        if (tstart>=1.d11.or.tstart<=-1.d10) then
     ...
     &     mem(1)(1:lmem(1))
        ...
        if (tstop>=1.d11.or.tstop<=-1.d10) then
        ...
        if (opt(4)==1) write (23,'(2a)') mem(40)(1:lmem(40)), &
     &     mem(7)(1:lmem(7))
        if (opt(4)==2) write (23,'(2a)') mem(40)(1:lmem(40)), &
     &     mem(8)(1:lmem(8))
        if (opt(4)==3) write (23,'(2a)') mem(40)(1:lmem(40)), &
     &     mem(9)(1:lmem(9))
!
...
! Write warning messages if necessary
        if (tstop<tstart) write (23,'(/,2a)') mem(121)(1:lmem(121)), &
     &     mem(123)(1:lmem(123))
        if (nbig<=0) write (23,'(/,2a)') mem(121)(1:lmem(121)), &
     &     mem(124)(1:lmem(124))
        if (nbig==nbod) write (23,'(/,2a)') mem(121)(1:lmem(121)), &
     &     mem(125)(1:lmem(125))
      endif
!
!------------------------------------------------------------------------------
...
! or whether massive Small bodies have different epochs than Big bodies.
      ...
      do j = nbig + 1, nbod
        if (m(j)/=0) then
          if (algor==1) call mio_err (23,mem(81),lmem(81),mem(94), &
...
! Check if central oblateness is being used with close-binary algorithm
      if (algor==11.and.(jcen(1)/=0.or.jcen(2)/=0.or.jcen(3) &
     &   /=0)) call mio_err (23,mem(81),lmem(81),mem(102),lmem(102), &
     &   ' ',1,mem(85),lmem(85))
!
! Check whether RCEN > RMAX or RMAX/RCEN is very large
      if (rcen>rmax) call mio_err (23,mem(81),lmem(81),mem(105), &
     &   lmem(105),' ',1,mem(85),lmem(85))
      if (rmax/rcen>=1.d12) write (23,'(/,2a,/a)') &
...
! Input/Output
      ...
      real(kind=8) time, tstart, en(3), am(3)
      character*80 outfile(4),mem(NMESS)
!
! Local
      integer unit, year, month
      real(kind=8) tmp0, tmp1, t1
      character*38 flog
      character*6 tstring
!
!------------------------------------------------------------------------------
!
      if (opt(5)==1) then
!	open (24, file=outfile(4), status='old', access='append')
	unit = 24
      else
	unit = 6
      endif
...
!
      tmp0 = 0.d0
      tmp1 = 0.d0
      if (en(1)/=0) tmp0 = (en(2) + en(3) - en(1)) / abs(en(1))
      if (am(1)/=0) tmp1 = (am(2) + am(3) - am(1)) / abs(am(1))
!
      if (opt(3)==1) then
        call mio_jd2y (time,year,month,t1)
        write (unit,flog) mem(64)(1:lmem(64)), year, month, t1, &
     &     mem(65)(1:lmem(65)), tmp0,mem(66)(1:lmem(66)), tmp1
      else
        if (opt(3)==0) t1 = time
        if (opt(3)==2) t1 = time - tstart
        if (opt(3)==3) t1 = (time - tstart) / 365.25d0
        write (unit,flog) mem(63)(1:lmem(63)), t1, tstring, &
     &     mem(65)(1:lmem(65)), tmp0, mem(66)(1:lmem(66)), tmp1
      endif
!      close(24)
!
...
! Input/Output
      ...
      real(kind=8) time,jcen(3),rcen,rmax,m(nbod),xh(3,nbod),vh(3,nbod)
      real(kind=8) s(3,nbod),rho(nbod)
      character*80 outfile
      character*8 id(nbod)
!
! Local
      integer k, len, nchar
      real(kind=8) rhocgs,k_2,rfac,rcen_2,fr,fv,theta,phi,vtheta,vphi
      character*80 header,c(NMAX)
      character*8 mio_fl2c,mio_re2c
      character*5 fout
!
...
! Create the format list, FOUT, used when outputting the orbital elements
      ...
      len = 3  +  6 * nchar
      fout(1:5) = '(a  )'
      if (len<10) write (fout(3:3),'(i1)') len
      if (len>=10) write (fout(3:4),'(i2)') len
!
...
! Input/Output
      ...
      real(kind=8) time, tstart, rmax, en(3), am(3), jcen(3)
      real(kind=8) m(nbod), x(3,nbod), v(3,nbod), s(3,nbod)
      character*80 outfile, mem(NMESS)
      character*8 id(nbod)
!
...
! Input/Output
      ...
      real(kind=8) m(nbod), x(3,nbod), v(3,nbod), s(3,nbod)
      real(kind=8) rho(nbod), rceh(nbod), rcrit(nbod), ngf(4,nbod)
      character*8 id(nbod)
      character*80 outfile, mem(NMESS)
!
! Local
...
! Input/Output
      integer nbod,nbig
      real(kind=8) jcen(3),m(nbod),xh(3,nbod),vh(3,nbod),s(3,nbod),e,l2
!
! Local
      integer j,k,iflag,itmp(8)
      real(kind=8) x(3,NMAX),v(3,NMAX),temp,dx,dy,dz,r2,tmp,ke,pe,l(3)
      real(kind=8) r_1,r_2,r_4,r_6,u2,u4,u6,tmp2(4,NMAX)
!
!------------------------------------------------------------------------------
!
      ke = 0.d0
...
! Input/Output
      integer nbod,nbig
      real(kind=8) jcen(3),m(nbod),xh(3,nbod),vh(3,nbod)
!
! Local
      integer j,itmp(8),iflag
      real(kind=8) x(3,NMAX),v(3,NMAX),temp,dx,dy,dz,r,d,a2,n,jac(NMAX)
      real(kind=8) tmp2(4,NMAX)
!
!------------------------------------------------------------------------------
!
      call mco_h2b(temp,jcen,nbod,nbig,temp,m,xh,vh,x,v,tmp2,iflag,itmp)
...
! Input/Output
      ...
      real(kind=8) time,tstart,h0,tol,jcen(3),m(nbod),x(3,nbod),v(3,nbod)
      real(kind=8) s(3,nbod),rceh(nbod),rho(nbod),epoch(nbod),ngf(4,nbod)
      character*8 id(nbod)
!
! Local
      integer j,k,l,nsml,nsofar,indx(NMAX),itemp,jtemp(NMAX)
      integer raflag,nce,ice(NMAX),jce(NMAX)
      real(kind=8) temp,epsml(NMAX),rtemp(NMAX)
      real(kind=8) h,hdid,tsmall,rphys(NMAX),rcrit(NMAX)
...
!
      if (abs(epsml(1)-tstart)<abs(epsml(nsml)-tstart)) then
        k = nsml + 1
        do j = 1, nsml / 2
          l = k - j
          temp = epsml(j)
...
!
      do j = nbig + 1, nbod
        nsofar = j - 1
        do while (abs(time-epoch(j))>tsmall)
          temp = epoch(j) - time
...
!...  Inputs and Outputs:
      ...
      real(kind=8) vx0,vy0,vz0

!...  Output
      integer iflg

!...  Internals:
      real(kind=8) x,y,z,vx,vy,vz,dt
      real(kind=8) f,g,fdot,c1,c2
      real(kind=8) c3,gdot
      real(kind=8) u,alpha,fp,r0,v0s
      real(kind=8) a,asq,en
      real(kind=8) dm,ec,es,esq,xkep
      real(kind=8) fchk,s,c
...
!
!	Input:
!	    dm		==> increment in mean anomaly M (real(kind=8) scalar)
!	    es,ec       ==> ecc. times sin and cos of E_0 (real(kind=8) scalars)
!
!       Output:
!            x          ==> solution to Kepler's difference eqn (real(kind=8) scalar)
!            s,c        ==> sin and cosine of x (real(kind=8) scalars)
!
...
!...    Internals
	...
	real(kind=8) dx
	real(kind=8) fac1,fac2,q,y
	real(kind=8) f,fp,fpp,fppp
...
!...  Inputs:
      real(kind=8) dt,r0,mu,alpha,u

!...  Outputs:
      real(kind=8) fp,c1,c2,c3
      integer iflg

!...  Internals:
      real(kind=8) s,st,fo,fn
...
!...  Inputs:
      real(kind=8) dt,r0,mu,alpha,u,s

!...  Outputs:
      real(kind=8) f

!...  Internals:
      real(kind=8)  x,c0,c1,c2,c3
...
!...  Inputs:
      real(kind=8) s,dt,r0,mu,alpha,u
...
!...  Internals:
      integer nc
      real(kind=8) x,c0,ds,s2
      real(kind=8) f,fpp,fppp,fdt
...
!...  Internals:
      real(kind=8) denom,a0,a1,a2,q,r,sq2,sq,p1,p2
...
!**********************************************************************
	...
	real(kind=8) function orbel_fget(e,capn)
...
!...  Internals:
	integer i,IMAX
	real(kind=8) tmp,x,shx,chx
	real(kind=8) esh,ech,f,fp,fpp,fppp,dx
	PARAMETER (IMAX = 10)
...
!**********************************************************************
	...
	real(kind=8) function orbel_fhybrid(e,n)
...
!...  Internals:
	real(kind=8) abn
        real(kind=8) orbel_flon,orbel_fget
...
!**********************************************************************
	...
	real(kind=8) function orbel_flon(e,capn)
...
!...  Internals:
	...
	real(kind=8) a,b,sq,biga,bigb
	real(kind=8) x,x2
	real(kind=8) f,fp,dx
	real(kind=8) diff
	real(kind=8) a0,a1,a3,a5,a7,a9,a11
	real(kind=8) b1,b3,b5,b7,b9,b11
	PARAMETER (IMAX = 10)
...
432	  format(1x,i3,3(2x,1PE22.15))
	  orbel_flon = x + dx
!   If we have converged here there's no point in going on
...
!**********************************************************************
	...
	real(kind=8) function orbel_zget(q)
