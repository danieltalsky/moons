...
!
      ...
      real(kind=8) time,teval,t0,t1,tprevious,rmax,rcen,rfac,rhocgs,temp
      real(kind=8) mcen,jcen(3),el(22,NMAX),s(3),is(NMAX),ns(NMAX),a(NMAX)
      real(kind=8) mio_c2re, mio_c2fl,fr,theta,phi,fv,vtheta,vphi,gm
      real(kind=8) x(3,NMAX),v(3,NMAX),xh(3,NMAX),vh(3,NMAX),m(NMAX)
      logical test
...
! What type elements does the user want?
  ...
  45  read (10,'(a250)') string
      if (string(1:1)==')') goto 45
      call mio_spl (250,string,nsub,lim)
      c2 = string(lim(1,nsub):(lim(1,nsub)+1))
      if (c2=='ce'.or.c2=='CE'.or.c2=='Ce') then
        centre = 0
      else if (c2=='ba'.or.c2=='BA'.or.c2=='Ba') then
        centre = 1
      else if (c2=='ja'.or.c2=='JA'.or.c2=='Ja') then
        centre = 2
      else
...
! Read parameters used by this programme
  ...
  50    read (10,'(a250)') string
        if (string(1:1)==')') goto 50
        call mio_spl (250,string,nsub,lim)
        c1 = string(lim(1,nsub):lim(2,nsub))
        if (j==1) read (string(lim(1,nsub):lim(2,nsub)),*) teval
        teval = abs(teval) * .999d0
        if (j==2.and.(c1=='d'.or.c1=='D')) timestyle = 0
        if (j==3.and.(c1=='y'.or.c1=='Y')) timestyle = timestyle+2
        if (j==4) call m_format (string,timestyle,nel,iel,fout,header, &
     &     lenhead)
      end do
!
! Read in the names of the objects for which orbital elements are required
  ...
  60  continue
        ...
        call mio_spl (250,string,nsub,lim)
        if (string(1:1)==')'.or.lim(1,1)==-1) goto 60
!
...
! Check if this is an old style input file
        if (ichar(check)==12.and.(style=='0'.or.style=='1'.or. &
     &     style=='2'.or.style=='3'.or.style=='4')) then
          write (*,'(/,2a)') ' ERROR: This is an old style data file', &
     &       '        Try running m_elem5.for instead.'
          stop
        end if
        if (ichar(check)/=12) goto 666
!
!------------------------------------------------------------------------------
!
...
! Create input format list
          if (precision==1) nchar = 2
          if (precision==2) nchar = 4
          if (precision==3) nchar = 7
          lenin = 3  +  6 * nchar
          fin(1:5) = '(a00)'
...
! Find the object on the master list
            unit(k) = 0
            do l = 1, nmaster
              if (id(k)==master_id(l)) unit(k) = master_unit(l)
            end do
!
...
! Convert to barycentric, Jacobi or close-binary coordinates if desired
          ...
          if (centre==1) call mco_h2b (jcen,nbod1,nbig1,temp,m,xh,vh, &
     &       x,v)
          if (centre==2) call mco_h2j (jcen,nbod1,nbig1,temp,m,xh,vh, &
     &       x,v)
          if (centre==0.and.algor==11) call mco_h2cb (jcen,nbod1, &
...
! Calculate true anomaly
            if (el(2,k)==0) then
              ...
              el(17,k) = acos(temp)
              if (sin(el(6,k))<0) el(17,k) = TWOPI - el(17,k)
            end if
! Calculate obliquity
...
! Convert time to desired format
          if (timestyle==0) t1 = time
          if (timestyle==1) call mio_jd_y (time,year,month,t1)
          if (timestyle==2) t1 = time - t0
          if (timestyle==3) t1 = (time - t0) / 365.25d0
!
! If output is required at this epoch, write elements to appropriate files
          if (firstflag==0.or.abs(time-tprevious)>=teval) then
            firstflag = 1
            tprevious = time
!
...
! If some objects remain on waiting list, read through input files again
      if (nwait>0) then
        do j = 1, nmaster
          if (master_unit(j)>=10) master_unit(j) = -1
          if (master_unit(j)==-2.and.nopen<NFILES) then
            nopen = nopen + 1
            nwait = nwait - 1
...
!
      ...
      rewind 10
!
      if (timestyle==0.or.timestyle==2) then
        write (10,'(/,a,f18.5,/)') ' Time (days): ',t1
      else if (timestyle==1) then
        write (10,'(/,a,i10,1x,i2,1x,f8.5,/)') ' Date: ',year,month,t1
      else if (timestyle==3) then
...
!
! Input/Output
      real(kind=8) g,mass,spin,rho,rote
!
! Local
      integer k
      real(kind=8) ss,s2,f,df,z,dz,tmp0,tmp1,t23
!
!------------------------------------------------------------------------------
!
      t23 = 2.d0 / 3.d0
...
!
! Input/Output
      real(kind=8) mu,q,e,i,p,n,l,x,y,z,u,v,w
!
! Local
...
! Check for inappropriate characters in the filename
      do j = 1, itmp + 1
        do k = 1, 5
          if (filename(j:j)==bad(k)) filename(j:j) = '_'
        end do
      end do
!
...
! Input/Output
      integer nbod,nbig
      real(kind=8) jcen(3),h,m(nbod),xh(3,nbod),vh(3,nbod),x(3,nbod),v(3,nbod)
!
! Local
      integer j
...
! Local
      integer j
      real(kind=8) msum,mvsum(3),temp,mbin,mbin_1,mtot_1
!
!------------------------------------------------------------------------------
!
      msum = 0.d0
...
! Input/Output
      integer nbod,nbig
      real(kind=8) jcen(3),h,m(nbig),xh(3,nbig),vh(3,nbig),x(3,nbig),v(3,nbig)
!
! Local
      integer j
...
! Input/Output
      integer nbod,nbig
      real(kind=8) jcen(3),h,m(nbod),x(3,nbod),v(3,nbod),xh(3,nbod),vh(3,nbod)
!
! Local
      integer j
!
...
!
      if (l<0.and.e<1) l = l + TWOPI
      if (l>TWOPI.and.e<1) l = mod (l, TWOPI)
!
!------------------------------------------------------------------------------
!
      return
...
! Identify the required elements
      ...
      do i = 1, nsub
        do j = 1, 22
          if (string(lim(1,i):lim(1,i))==elcode(j)) iel(i) = j
        end do
      end do
      nel = nsub
!
! For each element, see whether normal or exponential notation is required
      do i = 1, nsub
        ...
        do j = lim(1,i)+1, lim(2,i)
          if (formflag==0) pos = j
          if (string(j:j)=='.') formflag = 1
          if (string(j:j)=='e') formflag = 2
        end do
!
